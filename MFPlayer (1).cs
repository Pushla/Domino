...
namespace DominoC
{
    class MFPlayer
    {
        static public string PlayerName = "Jacob";
        static private List<MTable.SBone> lstHand;


        //=== Готовые функции =================
        // инициализация игрока
        static public void Initialize()
        {  lstHand = new List<MTable.SBone>();  }

        // Вывод на экран
        static public void PrintAll()
        {  MTable.PrintAll(lstHand);  }

        // дать количество доминушек
        static public int GetCount()
        {  return lstHand.Count;  }
        ...
        {
            int score = 0;
            foreach(MTable.SBone sb in lstHand)
            {
                score += sb.First + sb.Second;
            }
            return score;
        }
        ...
        //Проверка, есть ли число на кости
        static bool BoneContainsNum(MTable.SBone sb, int num)
        {
            if (sb.First == num || sb.Second == num)
            {
                return true;
            }
            else
        ...
        static MTable.SBone MaxBoneInList(List<MTable.SBone> lstBones)
        {
            int intMaxScore = 0;
            MTable.SBone outBone = new MTable.SBone();

            foreach (MTable.SBone b in lstBones)
            //Выбрать из имеющихся костей кость с наибольшей суммой
            {
                if (intMaxScore < (b.First + b.Second))
                {
                    intMaxScore = b.First + b.Second;
                    outBone = b;
                }
            }
